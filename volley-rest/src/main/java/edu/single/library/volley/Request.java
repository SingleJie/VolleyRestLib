...
public abstract class Request<T> implements Comparable<Request<T>> {
     ...
     *
     * @deprecated Use {@link #Request(int, String, edu.single.library.volley.Response.ErrorListener)}.
     */
    ...
    public String getUrl() {
        try {
            if (mMethod == Method.GET && getParams().size() != 0) {
                ...
                String temp = mUrl + extra;

                if(!EmptyUtils.emptyOfString(temp) && temp.lastIndexOf("&") == temp.length()-1){
    ...
    public static byte[] encodeParameters(Map<String, String> params, String paramsEncoding) {
        ...
        try {
            ...
            //删除最后一个&
            if(encodedParams.length()>0 && encodedParams.lastIndexOf("&") == encodedParams.length()-1){
                encodedParams.deleteCharAt(encodedParams.length()-1);
            }

            LogUtils.logEMsg("----------------- >" + encodedParams.toString());
